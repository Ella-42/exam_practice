...
{
	for (uint16_t fd = servfd + 1; fd <= lastfd; fd++)
...
int main(int ac, char *av[])
{
	...
	for (;;)
	{
		...
		for (uint16_t fd = servfd; ready > 0; fd++)
		{
			...
			{
				send(fd, "Message failed to send, buffer limit of 100001 exceeded\n", 56, MSG_NOSIGNAL);

				memset(clients[fd].buf, '\0', 100001);
				clients[fd].end = 0;
			...
			int recvd;
			if ((recvd = recv(fd, clients[fd].buf + clients[fd].end, 100001 - clients[fd].end, 0)) == -1) continue;
